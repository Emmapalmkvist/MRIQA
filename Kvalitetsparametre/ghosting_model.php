    ...
    while($row = mysqli_fetch_array($result2))
{
    ...
    foreach($scanner as $value)
    {
        ...
        $sql3 = "SELECT Ghostingmean, Model, Dato, Serienummer, Starttidspunkt, Ghostingbillede FROM Maaling WHERE Serienummer='$sn1' AND Dato BETWEEN '$startdato' AND '$slutdato' ORDER BY Dato";
        $scannerny = array();
		...
		type: "line",
        legend: "$sn",
        toolTipContent:"Dato: {label}<br/> Ghosting: {y}<br/>Serienummer: {sn} <br/> Starttidspunkt: {tidspunkt}{tidspunkt1}:{tidspunkt2}{tidspunkt3}",
		dataPoints: '.$dataset.'
	}';
    }
